...
export default function DemandeList() {
  ...
  useEffect(() => {
      if (newDemande.type === "CONGE" && newDemande.dateDebut && newDemande.dateFin) {
        const start = new Date(newDemande.dateDebut);
        const end = new Date(newDemande.dateFin);

        // Vérifie que la date de fin est après la date de début
        if (end >= start) {
          const diffTime = end.getTime() - start.getTime();
          const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24)) + 1; // inclusif
          setNewDemande(prev => ({ ...prev, nbJours: diffDays.toString() }));
        } else {
          setNewDemande(prev => ({ ...prev, nbJours: "0" }));
        }
      } else {
        setNewDemande(prev => ({ ...prev, nbJours: "" }));
      }
  ...
  try {
    // Préparer le payload : uniquement champs simples
    const payload: any = {
  ...
  const agences = Array.from(
    new Set(
      demandes
        .filter((d) => d.user?.agence) // sécurité
        .map((d) => d.user!.agence!.nom_agence) // ou d.user.agenceName selon ta structure
    )
  );
  ...
  const filteredDemandes = demandes.filter((d) => {
    ...
    const matchesAgence =
      agenceFilter === "all" ||
      d.user?.agence?.nom_agence === agenceFilter; // adapte selon ta structure
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent className="max-w-md">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {newDemande.type === "CONGE" && (
                  <div>
                    <Label>Nombre de jours</Label>
                    <Input type="number" value={newDemande.nbJours} readOnly />
                  </div>
              )}
                  ...
                  <TableCell>
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                          ...
                          {d.status === "APPROUVE" && (
                            <DropdownMenuItem onClick={() => generatePDF(d.id)}>
                              <Calendar className="w-4 h-4 mr-2" /> Télécharger PDF
                            </DropdownMenuItem>
                          )}
                        </DropdownMenuContent>
                      </DropdownMenu>
